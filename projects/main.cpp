...
int main() {
    ...
    glBindTexture(GL_TEXTURE_2D, lPosition);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, 512, 512, 0, GL_RGB, GL_FLOAT, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, lPosition, 0);
    unsigned int lNormal;
    glGenTextures(1, &lNormal);
    glBindTexture(GL_TEXTURE_2D, lNormal);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, 512, 512, 0, GL_RGB, GL_FLOAT, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, lNormal, 0);
    unsigned int lFlux;
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT2, GL_TEXTURE_2D, lFlux, 0);
    ...
    while ((Lerr = glGetError()) != GL_NO_ERROR) {
        std::cout << "GL error: 0x" << std::hex << Lerr << std::dec << std::endl;
    }
    ...
    screenShader.setFloat("indirectScale", indirectScale);
    lowresShader.use();
    ...
    lowresShader.setMatrix4("lightView", lview);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // 创建ImGui控制面板
        {
            ImGui::Begin("indirect light scale");
            
            ImGui::SliderFloat("Scale", &indirectScale, 0.0f, 10.0f);
